library(readxl)
yc=read_excel("C:/Users/u120753/Desktop/YieldCurve2020.xlsx",sheet = "yc",col_types = c("date",rep("numeric",9)))
...
p=ggplot(data=yc,aes(x=mat,y=value),group=mat)+
  ...
  #geom_point(data=yc[yc$mat=="5A",],aes(x=mat,y=value))+
  #geom_text(data=yc[yc$mat=="5A",],aes(label=value*100),hjust=0, vjust=-1,check_overlap = TRUE)
...
anim <- p +     transition_states(states = date,transition_length = 2)
  #enter_fade()+shadow_wake(0.1, size = 10, alpha = FALSE, colour = 'grey92')
